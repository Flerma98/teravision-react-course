...
import {useEvent, EventModel} from '../context/eventContext';

export default function EventsPage() {
    const router = useRouter();
    const {setEditingEvent} = useEvent();
    ...
    const editEvent = (event: EventModel) => {
        ...
        router.push(`events/form/${event.id}`);
    };
    ...
    return (
        <div>
            ...
            <div className="space-y-4">
                {events.map((event) => (
                    <div key={event.id} className="bg-gray-200 rounded-2xl p-4 flex items-center justify-between">
                        <div>
