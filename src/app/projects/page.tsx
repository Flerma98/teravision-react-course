...
export default function ProjectsPage() {
  ...
  const deleteProject = async (id: number) => {
    ...
    // If the page becomes empty, go back one page and then fetch.
    const nextLocal = projects.filter(p => p.id !== id);
    if (nextLocal.length === 0 && page > 1) {
      setPage(p => Math.max(1, p - 1));
      return; // effect will refetch due to page change
    }

    // Refresh same page to keep server-side pagination coherent
    try {
      const result = await fetchProjects({ pageNumber: page, pageSize, showAll: false });
      setProjects(result.items);
      setTotalPages(result.totalPages || Math.max(1, Math.ceil(result.totalCount / result.pageSize)));
    } catch {
      // Silent fallback: keep local state if refresh fails
    }
  };
