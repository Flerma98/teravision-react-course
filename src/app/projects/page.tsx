...
export default function ProjectsPage() {
    ...
    // Minimal pagination state to avoid breaking existing tests
    const [page, setPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);

    // Heuristic: if received exactly pageSize items, assume there might be more pages
    const hasPrev = page > 1;
    const hasNext = useMemo(() => projects.length === pageSize, [projects.length, pageSize]);
    ...
    const deleteProject = async (id: number) => {
        ...
        // Update local items; if page is now empty and page > 1, go back a page
        setProjects(prev => {
            const next = prev.filter(project => project.id !== id);
            if (next.length === 0 && page > 1) setPage(p => Math.max(1, p - 1));
            return next;
        });
    };
    ...
    return (
        <div>
            ...
            {/* Visual search kept for test parity; not wired to API to avoid changing contract */}
                ...
                {projects.map((project) => (
                    <div key={project.id} className="bg-gray-200 rounded-2xl p-4 flex items-center justify-between">
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => editProject(project)} aria-label={`Edit project ${project.name}`}>
                            ...
                            <button onClick={() => deleteProject(project.id)} aria-label={`Delete project ${project.name}`}>
